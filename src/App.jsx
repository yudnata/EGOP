  ...
  if (page.startsWith('masyarakat-')) {
    ...
    return (
      ...
      >
        ...
        {activePage === 'permohonan' && <PermohonanSuratPage user={DUMMY_USER} />}
        ...
        {activePage === 'pengaturan' && (
          <div className="bg-white border border-gray-200 rounded-xl p-6 max-w-xl mx-auto shadow-sm">
            <h2 className="text-lg font-bold text-gray-900 mb-2">Pengaturan Akun</h2>
            <p className="text-xs text-gray-400 mb-6">Kelola konfigurasi privasi, keamanan, dan notifikasi akun Anda.</p>
            <div className="space-y-4">
              <button onClick={handleLogout} className="px-4 py-2 bg-red-600 hover:bg-red-700 text-white font-bold rounded-lg text-xs transition-colors">
  ...
  if (page.startsWith('kaling-')) {
    ...
    return (
      ...
      >
        ...
        {activePage === 'settings' && (
          <div className="bg-white border border-gray-200 rounded-xl p-6 max-w-xl mx-auto shadow-sm">
            <h2 className="text-lg font-bold text-gray-900 mb-2">Pengaturan Kaling</h2>
            <p className="text-xs text-gray-400 mb-6">Konfigurasi panel kontrol Kepala Lingkungan.</p>
            <button onClick={handleLogout} className="px-4 py-2 bg-red-600 hover:bg-red-700 text-white font-bold rounded-lg text-xs transition-colors">
              Logout
            </button>
          </div>
  ...
  if (page.startsWith('kelurahan-')) {
    ...
    return (
      <KelurahanLayout
        ...
        user={DUMMY_LURAH}
      >
        {activePage === 'dashboard' && <KelurahanDashboardPage onNavigate={(id) => setPage(`kelurahan-${id}`)} />}
        ...
        {activePage === 'settings' && (
          <div className="bg-white border border-gray-200 rounded-xl p-6 max-w-xl mx-auto shadow-sm">
            <h2 className="text-lg font-bold text-gray-900 mb-2">Pengaturan Kelurahan</h2>
            <p className="text-xs text-gray-400 mb-6">Konfigurasi panel kontrol Kepala Kelurahan.</p>
            <button onClick={handleLogout} className="px-4 py-2 bg-red-600 hover:bg-red-700 text-white font-bold rounded-lg text-xs transition-colors">
              Logout
            </button>
          </div>
  ...
  if (page.startsWith('admin-')) {
    ...
    return (
      <AdminLayout
        ...
        user={DUMMY_ADMIN}
      >
        {activePage === 'dashboard' && <AdminDashboardPage onNavigate={(id) => setPage(`admin-${id}`)} />}
        {activePage === 'staf' && <ManajemenStafPage />}
        {activePage === 'layanan' && <ManajemenLayananPage />}
        {activePage === 'log' && <LogSistemPage />}
        {activePage === 'settings' && (
          <div className="bg-white border border-gray-200 rounded-xl p-6 max-w-xl mx-auto shadow-sm">
            <h2 className="text-lg font-bold text-gray-900 mb-2">Pengaturan Admin</h2>
            <p className="text-xs text-gray-400 mb-6">Konfigurasi kontrol sistem global dan hak akses.</p>
            <button onClick={handleLogout} className="px-4 py-2 bg-red-600 hover:bg-red-700 text-white font-bold rounded-lg text-xs transition-colors">
              Logout
            </button>
          </div>
