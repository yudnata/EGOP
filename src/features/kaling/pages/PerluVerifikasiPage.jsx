...
// Enriched dummy data that references actual service IDs
const DUMMY_PENDING = [
  {
    id: 1,
    name: 'Andi Wijaya',
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="px-7 pt-6 pb-4 border-b border-gray-100 flex items-start justify-between flex-shrink-0">
          <div>
            <h2 className="text-base font-bold text-gray-900">Detail Permohonan</h2>
            <p className="text-xs text-gray-400 mt-0.5">Verifikasi data dan dokumen warga sebelum menyetujui</p>
          </div>
        ...
        <div className="overflow-y-auto flex-1 px-7 py-5 space-y-5">
          ...
          <div className="border border-gray-200 rounded-xl p-5">
            ...
            <div className="grid grid-cols-2 gap-2">
              {(item.docs || []).map((doc) => (
                <div key={doc} className="border border-gray-200 rounded-lg px-3 py-2.5 flex items-center gap-2">
                  <svg className="w-4 h-4 text-blue-500 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m2.25 0H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z" />
                  </svg>
                  <span className="text-xs font-medium text-gray-700 truncate">{doc}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="border border-gray-200 rounded-xl p-5">
              ...
              {(service.flowSteps || []).map((step, i, arr) => (
                <Fragment key={step}>
                  <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded text-[10px] font-bold ${
                    ...
                    <span className={`w-4 h-4 rounded-full flex items-center justify-center text-[8px] font-extrabold ${
                      step === 'kaling' ? 'bg-amber-500 text-white' : 'bg-blue-600 text-white'
                    }`}>{i + 1}</span>
                    {FLOW_STEP_LABELS[step] || step}
                    {step === 'kaling' && <span className="text-[8px] ml-0.5">(Anda)</span>}
                  </span>
                  {i < arr.length - 1 && (
            ...
            {service.needsKelurahan && (
              <p className="text-[10px] text-blue-600 font-medium mt-2">
                ※ Setelah Anda setujui, surat pengantar akan diteruskan ke <strong>Kelurahan</strong> untuk proses selanjutnya.
              </p>
            )}
            {!service.needsKelurahan && (
              <p className="text-[10px] text-green-600 font-medium mt-2">
                ✓ Surat ini selesai di tingkat Lingkungan. Anda dapat langsung menerbitkan surat pengantar.
              </p>
            )}
          </div>
          ...
          {!showRejectForm && (
            <div className={`border rounded-xl p-5 transition-all ${
              ...
              <h4 className={`text-xs font-bold uppercase tracking-widest mb-2 flex items-center gap-1.5 ${
                showUploadError && !signedFile ? 'text-red-600' : 'text-amber-700'
              }`}>
              ...
              <p className="text-[11px] text-gray-500 mb-3 leading-relaxed">
                Cetak draft surat pengantar melalui tombol <strong>Preview Surat</strong> di bawah, bubuhkan tanda tangan basah & cap stempel, lalu foto/scan dan unggah di bawah ini sebagai bukti persetujuan fisik sebelum diteruskan.
              </p>

              <div className={`border-2 border-dashed rounded-xl p-4 flex flex-col items-center justify-center bg-transparent hover:bg-amber-50/5 transition-colors cursor-pointer relative ${
                showUploadError && !signedFile ? 'border-red-300' : 'border-amber-200'
              }`}>
                <input 
                  type="file" 
                  onChange={(e) => {
                ...
                <svg className={`w-8 h-8 mb-1 ${
                  showUploadError && !signedFile ? 'text-red-400' : 'text-amber-500'
                }`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M12 16.5V9.75m0 0l3 3m-3-3l-3 3M6.75 19.5a4.5 4.5 0 01-1.41-8.775 5.25 5.25 0 0110.233-2.33 3 3 0 013.758 3.848A3.752 3.752 0 0118 19.5H6.75z" />
                </svg>
                <span className="text-xs font-semibold text-gray-700 text-center px-2">
                  {signedFile ? signedFile.name : 'Pilih file Surat Pengantar yang sudah ditandatangani'}
                </span>
                <span className="text-[9px] text-gray-400 mt-0.5">PNG, JPG, atau PDF (Maks. 5MB)</span>
              </div>
              {showUploadError && !signedFile && (
                <p className="text-[10px] text-red-600 font-bold mt-2 flex items-center gap-1">
                  ⚠ Wajib mengunggah surat pengantar yang ditandatangani sebelum menyetujui.
                </p>
              )}
            </div>
          )}
          ...
          <div className="px-7 py-4 border-t border-gray-100 flex items-center justify-between flex-shrink-0">
            ...
            <div className="flex gap-2">
              ...
              >
                ...
                {service.needsKelurahan ? 'Setujui & Teruskan ke Kelurahan' : 'Setujui & Terbitkan Surat'}
              </button>
            </div>
          </div>
        )}
...
const PerluVerifikasiPage = ({ list, setList }) => {
  ...
  const getService = (serviceId) => SERVICES.find(s => s.id === serviceId) || { name: serviceId, flow: '', flowSteps: [], needsKelurahan: true };
  ...
  return (
    <div className="w-full space-y-6 pb-12">
      ...
      {/* Header */}
      <div>
        <h1 className="text-xl font-extrabold text-gray-900">Perlu Verifikasi</h1>
        <p className="text-gray-400 text-xs mt-0.5">Daftar permohonan warga yang menunggu verifikasi Anda sebagai Kepala Lingkungan.</p>
      </div>
                  ...
                  <tr key={item.id} className="hover:bg-gray-50/50 transition-colors">
                    <td className="px-6 py-4 flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-gray-400 mt-0.5">{item.block}</p>
                      </div>
                    </td>
              ...
              {activeList.length === 0 && (
                <tr>
                  <td colSpan={4} className="px-6 py-12 text-center text-gray-400">
                    ...
                    <p className="text-sm font-bold text-gray-500">Semua Permohonan Selesai</p>
                    <p className="text-xs text-gray-400 mt-0.5">Tidak ada permohonan yang perlu diverifikasi saat ini.</p>
                  </td>
                </tr>
              )}
            </tbody>
