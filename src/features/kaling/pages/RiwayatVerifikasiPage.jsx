...
const DUMMY_HISTORY = [
  { name: 'IVAN FROM CHINA', block: 'Blok A No. 12', service: 'Surat Pengantar KTP', date: '12 Okt 2023, 14:20', status: 'Disetujui' },
  { name: 'Siti Pertiwi', block: 'Blok C No. 04', service: 'Domisili Usaha', date: '11 Okt 2023, 09:15', status: 'Ditolak' },
  { name: 'Bambang Pamungkas', block: 'Blok B No. 22', service: 'Izin Keramaian', date: '10 Okt 2023, 16:45', status: 'Disetujui' },
  { name: 'Dewi Wahyuni', block: 'Blok D No. 01', service: 'Keterangan Pindah', date: '09 Okt 2023, 11:30', status: 'Disetujui' },
  { name: 'Rizky Kurniawan', block: 'Blok A No. 05', service: 'Surat Keterangan Kematian', date: '08 Okt 2023, 10:05', status: 'Ditolak' },
  { name: 'Rizky Kurniawan', block: 'Blok A No. 05', service: 'Surat Keterangan Kematian', date: '08 Okt 2023, 10:05', status: 'Ditolak' },
  { name: 'Rizky Kurniawan', block: 'Blok A No. 05', service: 'Surat Keterangan Kematian', date: '08 Okt 2023, 10:05', status: 'Ditolak' }
];
...
const RiwayatVerifikasiPage = () => {
  ...
  return (
    <div className="w-full space-y-6 pb-12">
      {/* Header */}
      <div>
        <h1 className="text-xl font-extrabold text-gray-900">Riwayat Verifikasi</h1>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-xs font-semibold text-gray-500">
          ...
          <div>
            ...
            <input type="date" className="w-full px-3 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white" />
          </div>
        </div>
      </div>
                ...
                return (
                  <tr key={idx} className="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded border text-[9px] font-bold uppercase tracking-wide ${badgeStyle}`}>
                        <span className={`w-1.5 h-1.5 rounded-full ${item.status === 'Disetujui' ? 'bg-green-500' : 'bg-red-500'}`} />
                        {item.status}
                      </span>
                    </td>
