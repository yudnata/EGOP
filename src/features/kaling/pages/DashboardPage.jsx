  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-6 flex items-start justify-between shadow-sm">
      <div className="space-y-1">
        ...
        <p className={`text-xs font-medium ${type === 'alert' ? 'text-red-500' : 'text-gray-400'}`}>{sub}</p>
      </div>
    ...
    <div className="w-full space-y-6 pb-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <StatCard 
          ...
          sub={pendingList.length > 0 ? "Butuh tindakan segera" : "Semua surat selesai"} 
          type={pendingList.length > 0 ? "alert" : "info"}
          icon={
            <svg className={`w-6 h-6 ${pendingList.length > 0 ? 'text-red-500 animate-pulse' : 'text-gray-400'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="text-base font-bold text-gray-800">Antrian Verifikasi Terbaru</h2>
          <button 
              ...
              <tbody className="divide-y divide-gray-100 text-gray-700 font-medium">
                {pendingList.slice(0, 5).map((item) => (
                  <tr key={item.id} className="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4">{getServiceName(item.serviceId)}</td>
