...
const DUMMY_CITIZENS = [
  { name: 'Aditya Wijaya', relation: 'Kepala Keluarga', nik: '327501**********', house: 'Blok A / 12', status: 'Tetap', job: 'Pegawai Negeri Sipil' },
  { name: 'Siti Lestari', relation: 'Istri', nik: '327501**********', house: 'Blok A / 12', status: 'Tetap', job: 'Wiraswasta' },
  { name: 'Bambang Kusumo', relation: 'Kepala Keluarga', nik: '327502**********', house: 'Blok B / 05', status: 'Kontrak', job: 'Karyawan Swasta' },
  { name: 'Rina Nuraini', relation: 'Kepala Keluarga', nik: '327501**********', house: 'Blok C / 21', status: 'Tetap', job: 'Guru' },
  { name: 'Fajar Ardiansyah', relation: 'Anggota Keluarga', nik: '327501**********', house: 'Blok A / 04', status: 'Kontrak', job: 'Mahasiswa' },
  { name: 'Fajar Ardiansyah', relation: 'Anggota Keluarga', nik: '327501**********', house: 'Blok A / 04', status: 'Kontrak', job: 'Mahasiswa' },
  { name: 'Fajar Ardiansyah', relation: 'Anggota Keluarga', nik: '327501**********', house: 'Blok A / 04', status: 'Kontrak', job: 'Mahasiswa' },
  { name: 'Fajar Ardiansyah', relation: 'Anggota Keluarga', nik: '327501**********', house: 'Blok A / 04', status: 'Kontrak', job: 'Mahasiswa' }
];
  ...
  return (
    <div className="w-full space-y-6 pb-12">
      ...
      <div className="flex justify-between items-center">
        <h1 className="text-xl font-extrabold text-gray-900">Riwayat Verifikasi</h1> {/* Matching screenshot title exactly */}
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm space-y-4">
          ...
          <table className="w-full text-left border-collapse text-xs">
            ...
            <tbody className="divide-y divide-gray-100 text-gray-700 font-medium">
              {filteredCitizens.map((item, idx) => {
                const statusStyle = item.status === 'Tetap' 
                  ...
                  <tr key={idx} className="hover:bg-gray-50/50 transition-colors">
                    <td className="px-6 py-4 flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-gray-400 mt-0.5">{item.relation}</p>
                      </div>
                    </td>
                    <td className="px-6 py-4 font-mono text-gray-500">{item.nik}</td>
                    <td className="px-6 py-4 text-gray-500">{item.house}</td>
                    <td className="px-6 py-4">
                      <span className={`inline-flex px-2 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wide ${statusStyle}`}>
                        {item.status}
                      </span>
                    </td>
        ...
        <div className="flex items-center justify-between border-t border-gray-100 pt-4 text-xs font-semibold text-gray-400">
          ...
          <div className="flex gap-1">
            ...
            <button className="w-7 h-7 flex items-center justify-center border border-gray-200 rounded hover:bg-gray-50 text-gray-500">50</button>
            <button className="w-7 h-7 flex items-center justify-center border border-gray-200 rounded hover:bg-gray-50 text-gray-500">&gt;</button>
          </div>
        </div>
      </div>
