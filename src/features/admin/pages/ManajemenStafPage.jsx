...
const DUMMY_STAF = [
  { id: 1, name: 'Lurah Renon', email: 'lurah.renon@civiclink.id', role: 'Kelurahan', area: 'Kelurahan Renon', status: 'Aktif' },
  { id: 2, name: 'Bpk. Herman', email: 'herman.kaling@civiclink.id', role: 'Kaling', area: 'Banjar Anyar Renon', status: 'Aktif' },
  { id: 3, name: 'Lurah Sanur', email: 'lurah.sanur@civiclink.id', role: 'Kelurahan', area: 'Kelurahan Sanur', status: 'Aktif' },
  { id: 4, name: 'Ibu Ratna', email: 'ratna.kaling@civiclink.id', role: 'Kaling', area: 'Banjar Tegal Sanur', status: 'Aktif' },
  { id: 5, name: 'Bpk. Joko', email: 'joko.kaling@civiclink.id', role: 'Kaling', area: 'Banjar Kaja Renon', status: 'Nonaktif' },
];
  ...
  return (
    <div className="w-full space-y-6 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-xl font-extrabold text-gray-900">Manajemen Staf & Pengguna</h1>
          <p className="text-xs text-gray-400 mt-1">Kelola hak akses dan akun administrasi Lurah (Kelurahan) serta Kepala Lingkungan (Kaling).</p>
        </div>
        <button
            ...
            <tbody className="divide-y divide-gray-100 text-gray-700 font-medium">
              {filteredStaf.length > 0 ? (
                filteredStaf.map((staf) => (
                  <tr key={staf.id} className="hover:bg-gray-50/50 transition-colors">
                    <td className="px-6 py-4">
                      <p className="font-semibold text-gray-900">{staf.name}</p>
                      <p className="text-[10px] text-gray-400 mt-0.5">{staf.email}</p>
                    </td>
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-md border text-[9px] font-bold uppercase tracking-wide ${
                        staf.role === 'Kelurahan' 
                    ...
                    <td className="px-6 py-4">
                      ...
                      >
                        <span className={`w-1.5 h-1.5 rounded-full ${staf.status === 'Aktif' ? 'bg-emerald-500' : 'bg-red-500'}`} />
                        {staf.status}
                      </button>
                    </td>
        ...
        <div className="fixed inset-0 z-50 overflow-y-auto bg-gray-950/40 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white border border-gray-200 rounded-xl shadow-2xl max-w-md w-full overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
              <h3 className="text-sm font-bold text-gray-800">Tambah Akun Staf Baru</h3>
              <button 
            ...
            <form onSubmit={handleAddStaf} className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1.5">Email Staf</label>
                <input
                  type="email"
                  required
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1.5">Peran / Role</label>
                  <select
                    value={formRole}
                ...
                <div>
                  <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1.5">Wilayah Tugas</label>
                  <input
                    type="text"
                    required
                    placeholder={formRole === 'Kelurahan' ? 'Kel. Renon' : 'Banjar Anyar Renon'}
                    value={formArea}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1.5">Kata Sandi</label>
                  <input
                    type="password"
                    required
                    placeholder="••••••••"
                ...
                <div>
                  <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1.5">Ulangi Kata Sandi</label>
                  <input
                    type="password"
                    required
                    placeholder="••••••••"
