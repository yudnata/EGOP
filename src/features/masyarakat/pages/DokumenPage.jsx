...
const DOC_STYLES = {
  KTP: { bgClass: 'from-rose-500 to-red-600 text-white', name: 'KTP (Kartu Tanda Penduduk)' },
  KK: { bgClass: 'from-blue-500 to-cyan-600 text-white', name: 'KK (Kartu Keluarga)' },
  SIM: { bgClass: 'from-emerald-400 to-teal-600 text-white', name: 'SIM (Surat Izin Mengemudi)' },
  'Akta Kelahiran': { bgClass: 'from-indigo-500 to-purple-600 text-white', name: 'Akta Kelahiran' },
  NPWP: { bgClass: 'from-amber-400 to-amber-600 text-slate-900', name: 'NPWP' },
  'BPJS / KIS': { bgClass: 'from-teal-500 to-emerald-600 text-white', name: 'BPJS / KIS (Kesehatan)' },
  'Pas Foto': { bgClass: 'from-slate-700 to-slate-900 text-white', name: 'Pas Foto Resmi' },
  'Ijazah': { bgClass: 'from-violet-500 to-fuchsia-600 text-white', name: 'Ijazah Terakhir' },
  'Lainnya': { bgClass: 'from-gray-500 to-gray-700 text-white', name: 'Dokumen Lainnya' }
};
...
const DokumenPage = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const styleInfo = DOC_STYLES[docType] || { bgClass: 'from-gray-500 to-gray-700 text-white', name: docName };
    const newDoc = {
  ...
  return (
    <div className="w-full space-y-6 pb-8">
      ...
      <div>
        ...
        <p className="text-gray-400 text-xs mt-0.5">Kelola, lihat, dan unduh seluruh dokumen kependudukan Anda secara digital dan aman.</p>
      </div>
            ...
            {filteredPersonalDocs.map((doc) => (
              ...
              >
                ...
                <div className="p-4 bg-gray-50 border-b border-gray-100 flex items-center justify-center">
                  <div className={`w-full aspect-[1.58/1] rounded-xl bg-gradient-to-br p-3.5 shadow-md flex flex-col justify-between relative overflow-hidden ${doc.bgClass}`}>
                    ...
                    <div className="flex items-start justify-between relative z-10">
                      <div className="flex items-center gap-1.5">
                        <div className="w-5 h-5 bg-white/20 rounded flex items-center justify-center backdrop-blur-sm">
                          <svg className="w-3 h-3 text-current" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                            <path strokeLinecap="round" strokeLinejoin="round" d="M15 9h3.75M15 12h3.75M15 15h3.75M4.5 19.5h15a2.25 2.25 0 002.25-2.25V6.75A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25v10.5A2.25 2.25 0 004.5 19.5z" />
                          </svg>
                        </div>
                        <span className="text-[9px] font-bold tracking-widest uppercase opacity-90">REPUBLIK INDONESIA</span>
                      </div>
                    ...
                    <div className="relative z-10 flex gap-3 items-end">
                      ...
                      <div className="flex-1 min-w-0 leading-none space-y-1">
                        <p className="text-[7px] uppercase font-bold tracking-wider opacity-60">Identity Card</p>
                        <p className="text-[10px] font-extrabold tracking-wide truncate uppercase">{doc.name.split(' ')[0]}</p>
                        <div className="h-0.5 w-8 bg-white/30 rounded"></div>
                        <p className="text-[8px] font-mono tracking-wider opacity-85">{doc.number}</p>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-4 flex-1 flex flex-col justify-between">
                  <div>
                    <div className="flex items-center justify-between mb-1.5">
                      <h3 className="text-xs font-bold text-gray-800 truncate">{doc.name}</h3>
                    </div>
                    <p className="text-xs font-mono text-gray-400 mb-4">{doc.number}</p>
                  </div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <button className="p-2 border border-gray-200 rounded-lg text-gray-400 hover:text-blue-600 hover:border-blue-200 transition-colors" aria-label="Unduh Dokumen">
                    ...
                    <tr key={doc.id} className="hover:bg-gray-50/50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="inline-flex gap-2.5">
                          <button className="p-1.5 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded transition-all" aria-label="Lihat Dokumen">
                            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                              <path strokeLinecap="round" strokeLinejoin="round" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z" />
                              <path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                            </svg>
                          </button>
                          <button className="p-1.5 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded transition-all" aria-label="Unduh Dokumen">
                            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm transition-all">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-xl border border-gray-100 flex flex-col gap-4 relative animate-scaleIn">
            ...
            <div>
              <h3 className="text-base font-bold text-gray-900">Tambah Dokumen Pribadi</h3>
              <p className="text-[11px] text-gray-400 mt-1 leading-relaxed">
                Unggah dokumen kependudukan utama Anda untuk mempermudah pendaftaran dan mempercepat verifikasi permohonan surat.
              </p>
            </div>

            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-[11px] font-bold text-gray-750 mb-1">Jenis Dokumen</label>
                <select 
                  value={docType}
                  onChange={(e) => {
              ...
              <div>
                <label className="block text-[11px] font-bold text-gray-750 mb-1">Nama Dokumen</label>
                <input 
                  type="text"
                  value={docName}
              ...
              <div>
                <label className="block text-[11px] font-bold text-gray-750 mb-1">Nomor Dokumen / NIK (Opsional)</label>
                <input 
                  type="text"
                  value={docNumber}
              ...
              <div>
                <label className="block text-[11px] font-bold text-gray-750 mb-1">Unggah File Dokumen</label>
                <div className="border-2 border-dashed border-gray-200 rounded-xl p-4 flex flex-col items-center justify-center bg-gray-50/50 hover:bg-gray-50 transition-colors cursor-pointer relative">
                  ...
                  <span className="text-xs font-semibold text-gray-700">
                    {uploadedFile ? uploadedFile.name : 'Pilih file atau drag & drop'}
                  </span>
                  <span className="text-[9px] text-gray-400 mt-0.5">PNG, JPG, PDF (Maks. 5MB)</span>
                </div>
              </div>
