...
const AjukanSuratPage = ({ user }) => {
  ...
  return (
    <div className="w-full space-y-6 pb-8 relative">
      ...
      {showToast && (
        <div className="fixed top-5 right-5 z-50 bg-green-600 text-white px-5 py-3 rounded-xl shadow-lg flex items-center gap-3 animate-bounce">
          ...
          <div>
            ...
            <p className="text-[10px] opacity-90">Silakan pantau status di menu Permohonan Surat Saya.</p>
          </div>
        </div>
      )}
      ...
      {/* Header */}
      <div>
        <h1 className="text-xl font-extrabold text-gray-900">Ajukan Surat Permohonan</h1>
        <p className="text-gray-400 text-xs mt-0.5">Pilih jenis layanan administrasi yang Anda butuhkan untuk memulai pengajuan dokumen baru.</p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-4 justify-between items-stretch sm:items-center border-b border-gray-100 pb-4">
        ...
        <div className="flex flex-wrap gap-2">
          {['Semua', 'Kaling Saja', 'Kelurahan', 'Kependudukan', 'Sosial & Ekonomi'].map((tab) => (
            <button
              key={tab}
      ...
      {filteredServices.length > 0 ? (
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-12 text-center">
          <svg className="w-12 h-12 text-gray-300 mx-auto mb-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 13.5h3.86a2.25 2.25 0 012.008 1.24l.885 1.77a2.25 2.25 0 002.007 1.24h1.98a2.25 2.25 0 002.007-1.24l.885-1.77a2.25 2.25 0 012.007-1.24h3.86m-18 0h18a2.25 2.25 0 002.25-2.25V6.75A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25v6.75a2.25 2.25 0 002.25 2.25z" />
          </svg>
          <h3 className="text-sm font-bold text-gray-700">Layanan Tidak Ditemukan</h3>
          <p className="text-xs text-gray-400 mt-1">Coba gunakan kata kunci lain atau pilih kategori yang berbeda.</p>
        </div>
      )}
