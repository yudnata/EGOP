...
const DUMMY_CITIZEN_QUEUE = [
  { name: 'Budi Santoso', gender: 'Laki-laki', age: 32, nik: '3275012345678901', rt: 'Banjar Anyar', date: '12 Okt 2023, 09:15', status: 'Disetujui' },
  { name: 'Siti Maemunah', gender: 'Perempuan', age: 28, nik: '3271044410950001', rt: 'Banjar Tegal', date: '11 Okt 2023, 14:40', status: 'Menunggu' },
  { name: 'Andi Herlambang', gender: 'Laki-laki', age: 45, nik: '3271046501780002', rt: 'Banjar Kaja', date: '11 Okt 2023, 10:22', status: 'Menunggu' },
  { name: 'Dewi Wahyuni', gender: 'Perempuan', age: 22, nik: '3271046108910005', rt: 'Banjar Kelod', date: '10 Okt 2023, 16:55', status: 'Menunggu' },
  { name: 'Rizky Junaedi', gender: 'Laki-laki', age: 38, nik: '3271041903840004', rt: 'Banjar Anyar', date: '10 Okt 2023, 11:30', status: 'Menunggu' }
];
...
const VerifikasiWargaPage = () => {
  ...
  return (
    <div className="w-full space-y-6 pb-12">
      {/* Header */}
      <div>
        <h1 className="text-xl font-extrabold text-gray-900">Verifikasi Akun Warga</h1>
      </div>

      {/* Stat Cards */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm space-y-1">
          <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Menunggu Verifikasi</p>
          <p className="text-2xl font-extrabold text-gray-900">24</p>
          <p className="text-[10px] text-gray-400">+5 hari ini</p>
        </div>
        <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm space-y-1">
          <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Diverifikasi (Bulan Ini)</p>
          <p className="text-2xl font-extrabold text-gray-900">142</p>
          <p className="text-[10px] text-blue-500 font-bold">12% dari bulan lalu</p>
        </div>
        <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm space-y-1">
          <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Total Warga Terdaftar</p>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm space-y-4">
        <div className="flex items-center justify-between border-b border-gray-50 pb-4">
          <h2 className="text-sm font-bold text-gray-800">Daftar Antrean Verifikasi</h2>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse text-xs">
            ...
            <tbody className="divide-y divide-gray-100 text-gray-700 font-medium">
              {list.map((item, idx) => {
                const badgeStyle = item.status === 'Disetujui' 
                  ...
                  <tr key={idx} className="hover:bg-gray-50/50 transition-colors">
                    <td className="px-6 py-4 flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-gray-400 mt-0.5">{item.gender}, {item.age} Thn</p>
                      </div>
                    </td>
                    <td className="px-6 py-4 font-mono text-gray-500">{item.nik}</td>
                    <td className="px-6 py-4 text-gray-500">{item.rt}</td>
                    <td className="px-6 py-4 text-gray-400">{item.date}</td>
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded border text-[9px] font-bold uppercase tracking-wide ${badgeStyle}`}>
                    ...
                    <td className="px-6 py-4 text-right">
                      {item.status === 'Menunggu' ? (
                        <button 
                          onClick={() => { setSelectedWarga(item); setSelectedIndex(idx); }}
        ...
        <div className="flex items-center justify-between border-t border-gray-100 pt-4 text-xs font-semibold text-gray-400">
          ...
          <div className="flex gap-1">
            ...
            <button className="w-7 h-7 flex items-center justify-center bg-slate-900 text-white rounded">1</button>
            <button className="w-7 h-7 flex items-center justify-center border border-gray-200 rounded hover:bg-gray-50 text-gray-500">2</button>
            <button className="w-7 h-7 flex items-center justify-center border border-gray-200 rounded hover:bg-gray-50 text-gray-500">3</button>
            <button className="w-7 h-7 flex items-center justify-center border border-gray-200 rounded hover:bg-gray-50 text-gray-500">&gt;</button>
          </div>
        </div>
      </div>
      ...
      {selectedWarga && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm transition-all animate-fadeIn">
          <div className="bg-white rounded-2xl max-w-lg w-full p-6 shadow-xl border border-gray-100 flex flex-col gap-4 relative animate-scaleIn">
            {/* Close button */}
            <button 
              onClick={() => { setSelectedWarga(null); setSelectedIndex(-1); }} 
            ...
            <div>
              <h3 className="text-base font-bold text-gray-900">Review Verifikasi Akun</h3>
              <p className="text-xs text-gray-400 mt-0.5">Periksa keabsahan data NIK dan berkas identitas pemohon sebelum memberikan persetujuan akun.</p>
            </div>
            ...
            <div className="border border-gray-200 rounded-xl p-4 space-y-3 bg-transparent">
              <div className="flex items-center gap-3 pb-3 border-b border-gray-100">
                <div className="w-10 h-10 border border-blue-500 rounded-full flex items-center justify-center text-xs font-bold text-blue-600">
                  {selectedWarga.name.split(' ').map(n => n[0]).join('')}
                </div>
                <div>
                  <h4 className="text-sm font-bold text-gray-800">{selectedWarga.name}</h4>
                  <p className="text-[10px] text-gray-400">{selectedWarga.gender}, {selectedWarga.age} Tahun — Terdaftar pada {selectedWarga.date}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-y-3 gap-x-4 text-xs">
                ...
                <div className="col-span-2">
                  ...
                  <span className="text-gray-700 font-medium">Banjar {selectedWarga.rt}, Kelurahan Panjer, Denpasar Selatan, Bali</span>
                </div>
              </div>
            </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="border border-gray-200 rounded-lg p-2.5 flex items-center gap-2 bg-transparent hover:bg-gray-50 transition-colors cursor-pointer">
                  ...
                  <div className="min-w-0 flex-1">
                    <span className="text-[10px] font-bold text-gray-700 block truncate">KTP_{selectedWarga.name.replace(/\s+/g, '')}.jpg</span>
                    <span className="text-[9px] text-gray-400">Foto KTP (1.2 MB)</span>
                  </div>
                </div>
                ...
                <div className="border border-gray-200 rounded-lg p-2.5 flex items-center gap-2 bg-transparent hover:bg-gray-50 transition-colors cursor-pointer">
                  ...
                  <div className="min-w-0 flex-1">
                    <span className="text-[10px] font-bold text-gray-700 block truncate">KK_{selectedWarga.name.replace(/\s+/g, '')}.pdf</span>
                    <span className="text-[9px] text-gray-400">Scan Kartu Keluarga (2.4 MB)</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            {selectedWarga.status === 'Menunggu' ? (
              ...
              <div className="pt-2 text-center">
                <span className={`inline-flex px-4 py-2 border text-xs font-bold rounded-lg ${
