const DUMMY_QUEUE = [
  { name: 'Rahmat Hidayat', nik: '3275012304880002', service: 'SKTM Pendidikan', status: 'Menunggu' },
  { name: 'Lina Marlina', nik: '3275014506920005', service: 'Surat Domisili', status: 'Menunggu' }
];
...
const DUMMY_URGENT = [
  { priority: 'PRIORITAS TINGGI', time: '10 Menit lalu', title: 'Surat Keterangan Tidak Mampu', name: 'Bpk. Ahmad Suherman (Banjar Anyar)' },
  { priority: 'REGULER', time: '2 jam lalu', title: 'Surat Kematian', name: 'Kel. Ibu Sartika (Banjar Kelod)' },
  { priority: 'REGULER', time: '3 jam lalu', title: 'Pindah Domisili', name: 'Sdr. Rizky Pratama (Banjar Kaja)' }
];
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-5 flex flex-col justify-between shadow-sm">
      <div className="flex justify-between items-start">
        ...
        {sub && (
          <span className={`text-[10px] font-extrabold px-1.5 py-0.5 border rounded ${type === 'alert' ? 'text-red-600 border-red-400' : 'text-gray-500 border-gray-300'}`}>
            {sub}
          </span>
        )}
      </div>
...
const KelurahanDashboardPage = ({ onNavigate }) => {
  return (
    <div className="w-full space-y-6 pb-12">
      {/* Header */}
      <div>
        <h1 className="text-xl font-extrabold text-gray-900">Dashboard Admin Kelurahan</h1>
      </div>

      {/* Stats row */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <StatCard label="Menunggu Validasi" value="24" sub="+12%" type="alert" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-2 space-y-6">
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-bold text-gray-800">Tren Pengajuan Surat</h3>
            ...
            <div className="w-full h-44 relative bg-gray-50/50 rounded-lg border border-gray-100 flex items-end p-4">
              <svg className="w-full h-full absolute inset-0 p-4" viewBox="0 0 100 40" preserveAspectRatio="none">
                <path d="M0,35 Q15,25 30,30 T60,15 T90,5 T100,2" fill="none" stroke="#2563eb" strokeWidth="2" strokeLinecap="round" />
                <path d="M0,35 Q15,25 30,30 T60,15 T90,5 T100,2 L100,40 L0,40 Z" fill="url(#gradient)" opacity="0.1" />
                <defs>
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm space-y-4">
            <h3 className="text-sm font-bold text-gray-800">Antrean Verifikasi</h3>
                  ...
                  {DUMMY_QUEUE.map((item, idx) => (
                    <tr key={idx} className="hover:bg-gray-50/50 transition-colors">
                      <td className="px-5 py-3">
                        ...
                        <p className="text-[10px] text-gray-400 font-mono mt-0.5">{item.nik}</p>
                      </td>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm space-y-4">
            <h3 className="text-sm font-bold text-gray-800">Aktivitas Terkini</h3>
            <div className="space-y-3">
              {DUMMY_ACTIVITIES.map((act, idx) => (
                <div key={idx} className="flex gap-3 items-start">
                  ...
                  <div>
                    <p className="text-xs font-bold text-gray-800">{act.service}</p>
                    <p className="text-[10px] text-gray-400 mt-0.5">{act.detail}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-bold text-gray-800">Validasi Mendesak</h3>
              <button 
                onClick={() => onNavigate('validasi-surat')}
              ...
              {DUMMY_URGENT.map((item, idx) => (
                <div key={idx} className="border border-gray-100 rounded-lg p-3 hover:border-gray-200 transition-colors">
                  <div className="flex justify-between items-center mb-1">
                    <span className={`text-[8px] font-bold uppercase tracking-wider ${item.priority === 'PRIORITAS TINGGI' ? 'text-red-600' : 'text-gray-400'}`}>
                      {item.priority}
                    </span>
                    <span className="text-[8px] text-gray-400 font-mono">{item.time}</span>
                  </div>
